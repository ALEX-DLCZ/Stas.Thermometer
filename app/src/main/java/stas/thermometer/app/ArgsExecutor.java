...
public class ArgsExecutor {
    ...
    public ArgsExecutor(String[] args) throws fatalException {
        ...
        for ( ValueType type : ValueType.values() ) {
            if ( readedConfigurationMap.containsKey(type.getType()) ) {
                try {
                    ...
                    AggregatorMain aggregatorInstance = new AggregatorMain(type.getType(), probeType.getDeclaredConstructor(List.class).newInstance(profil), type.getDelta());
        ...
        this.connectionString = "jdbc:mysql://" + readedConfigurationMap.get("BD").get("IpServer") + ":" + readedConfigurationMap.get("BD").get("PortServer") + "/" + readedConfigurationMap.get("BD").get("User") + "?" + "user=" + readedConfigurationMap.get("BD").get("User") + "&" + "password=" + readedConfigurationMap.get("BD").get("Pws");
    }
